...
# --- 페이지 기본 설정 ---
st.set_page_config(page_title="프로젝트A 시뮬레이터 V7", layout="wide")

st.title("🚀 프로젝트A 3개년 매출 & 지표 시뮬레이터 (V7)")
...
st.info("각 월별로 결제 비율과 객단가를 조정하세요. 환산된 ARPDAU와 지표가 실시간 반영됩니다.")
...
for i, row in df.iterrows():
    ...
    st.markdown(f"#### 📍 {m_str}")
    c1, c2 = st.columns(2)
    pr = c1.slider(f"결제 비율 (%)", 0.1, 20.0, 5.0, 0.1, key=f"v7_pr_{i}")
    ar = c2.slider(f"결제 객단가 (원)", 10000, 500000, 70000, 1000, key=f"v7_ar_{i}")
...
df["ARPDAU"] = df["ARPPU"] * (df["P.rate(%)"] / 100) # ARPDAU 계산 복구
...
st.sidebar.download_button(label="📥 결과 CSV 다운로드", data=csv, file_name=f"ProjectA_V7_Plan.csv", mime="text/csv")
